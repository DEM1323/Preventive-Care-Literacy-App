...
- The separately authorized, freshness-gated, no-store clinical reveals (`POST /api/v1/clinical/intake-records/current` and `POST /api/v1/clinical/intake-records/versions`) are the only staff HTTP seams that may return plaintext answers from an accepted Intake Record Version, and only after `staff_identities`, `staff_permission_grants`, `staff_sessions`, and `school_workspaces` are locked in that order and the current Staff Session, Clinical Permission, School Workspace, and password-plus-TOTP Authentication Freshness are rechecked through ciphertext retrieval, in-process decrypt, and the append-only audit decision. Historical reveal uses the version's frozen source School Configuration Release and Intake Form revision rather than the active release. Authentication Freshness is rechecked after decrypt and before success audit or return. The response must not be stored by caches, URLs, telemetry, or browser storage.
...
This note is the threat-model verification for issue #28 criterion 1, updated for the Student-private draft restore seam, issue #30's authorized clinical reveal seam, and issue #46's historical Intake Record Version reveal. It is not a regulatory assessment.
